...
3. `Claude Cowork Guiding Files/FRONTEND_SPEC.md`
   - Use `app/export/page.tsx`, `components/ADIPartsPanel.tsx`, and `components/RationalePanel.tsx`.
...
- `frontend/components/RationalePanel.tsx`
- `frontend/lib/api.ts`
...
1. Wire export routers into `backend/main.py`.
   - Include ADI and rationale routers at `/api/export`.
...
1. Extend `frontend/lib/api.ts`.
   - Export `getBOM`, `getRationale`, and `downloadStlUrl`.
...
3. Implement `frontend/components/RationalePanel.tsx`.
...
5. Add demo polish.
   - The page should make Analog Devices and Design Rationale obvious.
...
- Confirm BOM and Rationale panels render without any other slice being complete.
